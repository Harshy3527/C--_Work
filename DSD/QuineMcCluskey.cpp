...
{ // checking if any object Term has a "bits" = t.bits
...
vector<Term> generatePrimeImplicants()
{
            ...
            for (int j = i + 1; j < terms.size(); j++)
            {
                ...
                { // merging success
                    ...
                    if (!contains(newTerms, t))
                        newTerms.push_back(t); // add new term to the newTerms if its not already there
                }
            }
        }

        for (auto &t : terms)
        { // if a term couldn't be merged, it is a prime implicant also
    ...
    for (auto &t : terms)
    {
        if (!contains(primes, t))
            primes.push_back(t); // add leftover prime implicants to primes
    }
...
int main()
{
    ...
    while (input != 0)
    {
        ...
        cout << endl;
        cout << "Press 0 to exit and and any other number to run again :- " << endl;
        cin >> input;
    }
}
